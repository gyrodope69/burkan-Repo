...
#     print(f"[Gemini] Prompt sent: {repr(prompt)}")

#     if not prompt.strip():
#         print("[Gemini] Empty prompt, skipping call")
#         return
...
#             print(f"The token: {token} (type: {type(token)})")
...
#                         print(f"[Gemini] Error sending token over websocket: {send_err}")
#                 reply_text += token
#             else:
#                 print("[Gemini] Skipped chunk with empty or invalid text")

#         if reply_text and isinstance(reply_text, str) and reply_text.strip() != "" and websocket.client_state.value == 1:
#             await stream_elevenlabs_ws(reply_text.strip(), websocket)
#             context.append({"role": "assistant", "content": reply_text})
#             print(f"[Gemini] Full reply: {reply_text}")
...
#         if websocket.client_state.value == 1:
#             try:
#                 await websocket.send_text(json.dumps({"type": "llm_token", "text": error_msg}))
#             except Exception as send_err:
#                 print(f"[Gemini] Error sending error message over websocket: {send_err}")
...
# import json
# from .tts_client import stream_elevenlabs_ws
# from .tts_sync_client import text_to_speech_stream  # new fallback TTS
...
#                 await stream_elevenlabs_ws(reply_text.strip(), websocket)
#             except Exception as e:
#                 print(f"[Gemini] Realtime TTS failed. Fallback: {e}")
#                 try:
#                     audio_stream = text_to_speech_stream(reply_text.strip())
#                     await safe_ws_send(websocket, audio_stream.read(), send_bytes=True)
#                 except Exception as err:
#                     print(f"[Gemini] Sync TTS failed: {err}")
